...
export class Admin extends Component {
  ...
  postLocation(location) {
    locationApi.postLocation(this.state.city._id, location).then((results) => {
        this.fetchCity();
    });        
  }

  deleteLocation(locationId) {
    locationApi.deleteLocation(this.state.city._id, locationId).then((results) => {
      this.fetchCity();
    });        
  }
  ...
  render() {
    if (!this.state.city) {
      ...
      const neighborhoods = !this.state.city.neighborhoods ? null : this.state.city.neighborhoods.map((neighborhood) => {
        return (<NeighborhoodListItem cityId={this.state.city._id} deleteNeighborhood={this.deleteNeighborhood} key={neighborhood._id} neighborhood={neighborhood} /> );      
      });
      
      const locations = !this.state.city.locations ? null : this.state.city.locations.map((location) => {
        return <LocationListItem key={location._id} cityId={this.state.city._id} location={location} deleteLocation={this.deleteLocation} />   
      })
